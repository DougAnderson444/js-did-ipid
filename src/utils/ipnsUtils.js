...
async function publish(cid) {
    ...
    if (!remListSubs.includes(`/ipns/${keys.id}`)) { console.log(`[Fail] !Name.Pubsub.subs ${keys.id}`); }

    // Publish will send a pubsub msg to the server to update their ipns record
    console.log(
    `Pushing /ipfs/${cid.toBaseEncodedString()} to ${keys.name} /ipns/${
      keys.id
    }`
    );
    ...
    ipfsAPI.pin.add(cid, { recursive: false }).then((pinset) => {
