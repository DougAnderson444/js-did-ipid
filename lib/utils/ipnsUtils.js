...
async function publish(cid) {
    if (!cid) { throw new Error('Missing cid to publish'); }
    if (!ipfsAPI) { throw new Error('Connect to a go-server node first'); }
    if (!ipfsAPI.name.pubsub.state() || !ipfsBrowser.name.pubsub.state()) { throw new Error('IPNS Pubsub must be enabled on bother peers, use --enable-namesys-pubsub'); }
    ...
    if (!remListSubs.includes(`/ipns/${keys.id}`)) { console.log(`[Fail] !Name.Pubsub.subs ${keys.id}`); } // Publish will send a pubsub msg to the server to update their ipns record

    console.log(`Pushing /ipfs/${cid.toBaseEncodedString()} to ${keys.name} /ipns/${keys.id}`);
    const results = await ipfsBrowser.name.publish(`/ipfs/${cid.toBaseEncodedString()}`, {
        resolve: false,
        key: keyName,
    });

    console.log(`Pushed ${results.name} to ${results.value}`); //

    const name = await last(ipfsAPI.name.resolve(keys.id, {
        stream: false,
    }));

    if (name == cid) {
        console.log(`Look at that! /ipns/${keys.id} resolves to ${cid}`);
    } else {
        console.log(`Error, resolve did not match ${name} !== ${cid}`);
    }

    ipfsAPI.pin.add(cid, {
        recursive: false,
    }).then((pinset) => {
        console.log(`pinned ${JSON.stringify(pinset, null, 2)}`);
    });
}
